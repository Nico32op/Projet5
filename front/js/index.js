...
function addCard(image, prod, prix, id) {
  /*partie que permet d'insérer le texte dans la partie html*/
  const card =
    /*?{id} permet de rajouter l'id dans la barre uril*/
    `
  ...
  cardlist.insertAdjacentHTML(
    "beforeend",
    card
  ); /*permet d'insérer ma const "card" dans ma div cardlist du html*/
}
...
request
  .then(async (response) => {
    ...
    try {
      ...
      for (let i = 0; i < donnejson.length; i++) {
        /*boucle qui me permet d'ajouter les arguments de ma fonction addcard jusqu'à la fin du tableau (const response qui contient tous le json)*/
        addCard(
